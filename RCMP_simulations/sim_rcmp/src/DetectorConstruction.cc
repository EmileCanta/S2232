...
//////ARIES/////////
void DetectorConstruction::AddDetectionSystemAries(G4int) {
  if(fLogicWorld == NULL) {
    Construct();
  }

  DetectionSystemAries* pAries = new DetectionSystemAries() ;
  pAries->Build();
  pAries->PlaceDetector( fLogicWorld ) ;
}
...
void DetectorConstruction::AddDetectionSystemQED() {
	if(fLogicWorld == nullptr) {
		Construct();
	}

		DetectionSystemQED* pQED = new DetectionSystemQED() ;
		pQED->Build();
		pQED->PlaceDetector(fLogicWorld);
	fQED = true;
	}

/////RCMP DETECTORS///////

void DetectorConstruction::AddDetectionSystemRCMP() {
	if(fLogicWorld == nullptr) {
		Construct();
	}
...
bool DetectorConstruction::CheckVolumeName(G4String volumeName) {
	...
	if(volumeName.find("DSSSDpixelLog") != G4String::npos) return true;
...
///////////////////////////////////////////////////////////////////////////////////////////////
 //recangle1
	...
	if(volumeName.find("rect1log")!= G4String::npos){	
		result.systemID = 8060;
		return result;
	
	}
	if(volumeName.find("squarescintlog")!= G4String::npos){	
		result.systemID = 8060;
		return result;
	
	}
	if(volumeName.find("scinttrianglelog")!= G4String::npos){	
		result.systemID = 8060;
		return result;
	
	}
	if(volumeName.find("rect2log")!= G4String::npos){	
		result.systemID = 8060;
		return result;
	...
	if(volumeName.find("DSSSDpixelLog")!= G4String::npos){
		/////Assign hit from pixel to strip number/////
		
		///Currently looking at 1 DSSSD with RCMP definition, memonic RCS01XP%dX for junction RCS01XN%dX for ohmic/////////////
		////////// det number from 0 to 1023, with 
	
		//record pixel copy number, will be process to front back strip number at NTuple2EventTree
		
		//QED 1 DSSSD detNum0-1023 for inner distance along x-axis before griffin rotation
		//QED 2 DSSSD detNum1024-2047 for inner distance along - x-axis before griffin rotation
		//QED 3 DSSSD detNum2048-3071 for inner distance along y-axis before griffin rotation
		//QED 4 DSSSD detNum3072-4095 for inner distance along -y-axis before griffin rotation
		//QED 5 DSSSD detNum4096-5119 for outer distance along y-axis before griffin rotation
		//QED 5 DSSSD detNum5120-6143 for outer distance along -y-axis before griffin rotation
		
		
		
		
			// check if the detNum is good. 
			if (result.detectorNumber<=6144){
				result.systemID = 9000;
				return result;}
				//assign to PACES first not using it anyway
			else {result.systemID = 50;
			//If the detector construction have exceeding element which really shouldn't occur, dump them to PACES channels
				return result;}
