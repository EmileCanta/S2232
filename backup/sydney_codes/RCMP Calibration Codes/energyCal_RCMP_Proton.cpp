...
void energyCal_RCMP_Proton(int run_number, std::string options = "f") {
	...
	std::ifstream gin("/home/sydney/S2232_analysis/cal_files/proton_files/RCMP_gains_28267_proton.txt");  
	if(!gin.is_open()){std::cerr << "ERROR: Could not open RCMP_gains_28267_proton.txt\n";} 
    ...
    for (int i = 5; i <= NDET; i++) { 
		...
		for (int strip = 0; strip < NStrips; strip++) { //looping through all 64 crystals in data
			...
			for (int j = 0; j < NPEAKS; j++) {
				...
				charge_estimates[j] = gain_estimate*peak_energies[j] + 95;
				}
				else {
	...
	// Write new .cal file
	std::string calibration_file = RCMPOptions::calibration_directory_files_protons + "energy_calibration" + std::to_string(run_number) + ".cal";
